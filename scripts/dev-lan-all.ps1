...
Write-Host "Rajeeta ALL on IP: $lanIp"
...
Write-Host "Expo:   Scan the QR code from the Expo terminal with Expo Go"
...
Set-EnvLine "apps\patient-expo\.env" "EXPO_PUBLIC_API_URL" $apiUrl



foreach ($port in 3000, 3001, 3002, 8081) {
...
Start-Process powershell -ArgumentList @(
  ...
  "Set-Location '$((Get-Location).Path)'; npm run dev:expo"
